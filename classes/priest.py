...
CLASS_DATA = {
    ...
    "hit_dice": "1d6",
    "proficiencies": { "weapons": "Club, crossbow, dagger, mace, longsword, staff, warhammer", "armor": "All armor and shields" },
    "titles": [
        {"level": "1-2", "lawful": "Acolyte", "chaotic": "Initiate", "neutral": "Seeker"},
        {"level": "3-4", "lawful": "Crusader", "chaotic": "Zealot", "neutral": "Invoker"},
        {"level": "5-6", "lawful": "Templar", "chaotic": "Cultist", "neutral": "Haruspex"},
        {"level": "7-8", "lawful": "Champion", "chaotic": "Scourge", "neutral": "Mystic"},
        {"level": "9-10", "lawful": "Paladin", "chaotic": "Chaos Knight", "neutral": "Oracle"}
    ],
    "talent_table": [
        {"range": (2, 2), "effect": "Gain advantage on casting one spell you know"},
        {"range": (3, 6), "effect": "+1 to melee or ranged attacks"},
        {"range": (7, 9), "effect": "+1 to priest spellcasting checks"},
        {"range": (10, 11), "effect": "+2 to Strength or Wisdom stat"},
...
def roll_talent():
    ...
    return f"Talent (Roll {roll}): +1 to melee or ranged attacks"
...
def get_title(level, alignment):
    ...
    for entry in CLASS_DATA['titles']:
        ...
        if low <= level <= high:
            return entry.get(alignment_key, "Seeker")
    return "Seeker"
...
def apply_effects(character, gear_data):
    ...
    for i in range(1, level + 1):
        ...
        if is_stout: roll = max(roll, random.randint(1, 6))
        total_hp += max(1, roll + con_mod)
    character['hp']['max'] += total_hp
    character['hp']['current'] += total_hp

    lang = random.choice(["Celestial", "Diabolic", "Primordial"])
    if "languages" not in character:
        character["languages"] = []
    if lang not in character["languages"]:
        character["languages"].append(lang)
        
    if "Turn Undead" not in character.get('talents', []):
        character['talents'].append("Spell: Turn Undead")
        
    tier_1_spells = ["Cure Wounds", "Holy Weapon", "Light", "Protection From Evil", "Shield of Faith"]
    known_spells = random.sample(tier_1_spells, 2)
    for spell in known_spells:
        character['talents'].append(f"Spell: {spell}")

    if "Holy Symbol" not in character['inventory']:
        character['inventory'].append("Holy Symbol")
